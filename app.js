(function () {
  ...
  const defaultStorageKey = config.brand.persistence.defaultStorageKey;
  ...
  function bindControls() {
    [headline, story, category, $('fontSize'), $('kickerSize'), $('lineHeight'), $('contrast'), $('badgePosition'), $('imageZoom'), $('imagePosition'), $('textVertical'), $('textHorizontal'), $('textY'), $('textX'), $('logoX'), $('logoY'), $('logoSize'), $('logoOpacity'), $('shadowOpacity'), $('shadowCoverage'), $('shadowDirection'), $('shadowBlur')].forEach(control => control.addEventListener('input', handleControl));
    ...
    $('saveDefaults').addEventListener('click', saveDefaultSettings); $('resetDefaults').addEventListener('click', resetFactoryDefaults);
  ...
  function handleControl(event) { const id = event.target.id; if (id === 'category') state.category = category.value; if (id === 'fontSize') state.fontSize = Number(event.target.value); if (id === 'kickerSize') state.kickerSize = Number(event.target.value); if (id === 'lineHeight') state.lineHeight = Number(event.target.value); if (id === 'contrast') state.contrast = Number(event.target.value); if (id === 'badgePosition') state.badgePosition = event.target.value; if (id === 'imageZoom') state.imageZoom = Number(event.target.value); if (id === 'imagePosition') state.imagePosition = event.target.value; if (id === 'textVertical') state.textVertical = event.target.value; if (id === 'textHorizontal') state.textHorizontal = event.target.value; if (id === 'textY') state.textY = Number(event.target.value); if (id === 'textX') state.textX = Number(event.target.value); if (id === 'logoX') state.logoX = Number(event.target.value); if (id === 'logoY') state.logoY = Number(event.target.value); if (id === 'logoSize') state.logoSize = Number(event.target.value); if (id === 'logoOpacity') state.logoOpacity = Number(event.target.value); if (id === 'logoPreset') state.logoPreset = event.target.value; if (id === 'logoVisible') state.logoVisible = event.target.checked; if (id === 'shadowOpacity') state.shadowOpacity = Number(event.target.value); if (id === 'shadowCoverage') state.shadowCoverage = Number(event.target.value); if (id === 'shadowDirection') state.shadowDirection = event.target.value; if (id === 'shadowBlur') state.shadowBlur = Number(event.target.value); updateLabels(); render(); }
  function syncControls() { const values = { category: state.category, fontSize: state.fontSize, kickerSize: state.kickerSize, lineHeight: state.lineHeight, contrast: state.contrast, badgePosition: state.badgePosition, imageZoom: state.imageZoom, imagePosition: state.imagePosition, textVertical: state.textVertical, textHorizontal: state.textHorizontal, textY: state.textY, textX: state.textX, logoX: state.logoX, logoY: state.logoY, logoSize: state.logoSize, logoOpacity: state.logoOpacity, logoPreset: state.logoPreset, shadowOpacity: state.shadowOpacity, shadowCoverage: state.shadowCoverage, shadowDirection: state.shadowDirection, shadowBlur: state.shadowBlur };
    Object.keys(values).forEach(id => { const control = $(id); if (control) control.value = values[id]; }); $('logoVisible').checked = state.logoVisible; $('headline').value = state.headline || ''; $('story').value = state.story || ''; $('charCount').textContent = story.value.length; $('logoName').textContent = state.logoName || 'ZONE SPORT'; updateLabels(); }
  function serializeDefaultSettings() { return { headline: headline.value, story: story.value, category: state.category, variation: state.variation, textVertical: state.textVertical, textHorizontal: state.textHorizontal, textX: state.textX, textY: state.textY, fontSize: state.fontSize, kickerSize: state.kickerSize, lineHeight: state.lineHeight, badgePosition: state.badgePosition, logoVisible: state.logoVisible, logoPreset: state.logoPreset, logoX: state.logoX, logoY: state.logoY, logoSize: state.logoSize, logoOpacity: state.logoOpacity, logoName: state.logoName, logoData: state.logo ? state.logo.src : null, imageZoom: state.imageZoom, imagePosition: state.imagePosition, shadowOpacity: state.shadowOpacity, shadowCoverage: state.shadowCoverage, shadowDirection: state.shadowDirection, shadowBlur: state.shadowBlur, layers: { ...state.layers } }; }
  function saveDefaultSettings() { localStorage.setItem(defaultStorageKey, JSON.stringify(serializeDefaultSettings())); showToast('تم حفظ إعداداتك الافتراضية بنجاح!'); }
  function restoreDefaultSettings() { try { const saved = JSON.parse(localStorage.getItem(defaultStorageKey) || 'null'); if (!saved) return; Object.keys(saved).forEach(key => { if (key !== 'logoData' && key !== 'imageData' && key !== 'layers' && key in state) state[key] = saved[key]; }); if (saved.layers) state.layers = { ...state.layers, ...saved.layers }; state.headline = saved.headline || ''; state.story = saved.story || ''; state.logoName = saved.logoName || ''; if (saved.logoData) { const logo = new Image(); logo.onload = () => { state.logo = logo; render(); }; logo.src = saved.logoData; } } catch (error) { localStorage.removeItem(defaultStorageKey); } }
  function resetFactoryDefaults() { localStorage.removeItem(defaultStorageKey); window.location.reload(); }
  function updateLabels() { $('fontSizeValue').textContent = state.fontSize; $('kickerSizeValue').textContent = state.kickerSize + 'px'; $('lineHeightValue').textContent = state.lineHeight.toFixed(2); $('contrastValue').textContent = state.contrast + '%'; $('textYValue').textContent = state.textY + '%'; $('textXValue').textContent = state.textX + '%'; $('logoXValue').textContent = state.logoX + '%'; $('logoYValue').textContent = state.logoY + '%'; $('logoSizeValue').textContent = state.logoSize + 'px'; $('logoOpacityValue').textContent = state.logoOpacity + '%'; $('shadowOpacityValue').textContent = state.shadowOpacity + '%'; $('shadowCoverageValue').textContent = state.shadowCoverage + '%'; $('shadowBlurValue').textContent = state.shadowBlur + '%'; }
  ...
  function render() {
    ...
    drawBase(template, variation); if (state.layers.overlay) drawControlledOverlay((blockTop + blockHeight / 2) / canvas.height); if (state.layers.logo && state.logo) drawLogo();
    ctx.save(); ctx.direction = 'rtl'; ctx.textAlign = state.textHorizontal; const textX = canvas.width * state.textX / 100; let y = blockTop + 28;
    if (state.layers.headline) { ctx.fillStyle = template.accent; ctx.font = '700 ' + state.kickerSize + 'px Tajawal, Arial'; ctx.fillText(template.kicker, textX, y); y += state.kickerSize + 3; ctx.fillStyle = '#fff'; const lineStart = state.textHorizontal === 'right' ? textX - 100 : state.textHorizontal === 'left' ? textX : textX - 50; ctx.fillRect(lineStart, y, 100, 4); y += 85; ctx.font = '800 ' + title.size + 'px Tajawal, Arial'; ctx.fillStyle = '#fff'; title.lines.forEach(line => { ctx.fillText(line, textX, y); y += title.size * state.lineHeight; }); }
    ...
    ctx.restore(); updateLabels(); renderLayers();
  }
  init();
})();